...
def get_disk_utilization_percent(path: str | None = None) -> float | None:
    try:
        if path is None:
...
def get_max_disk_utilization_from_env() -> float | None:
...
class DiskUtilizationErrorHandlerTemplate(ABC):
    def __init__(self, max_disk_utilization: float | None, func: Callable[..., None], *args: Any, **kwargs: Any):
