...
if TYPE_CHECKING:
    ...
    from minfx.neptune_v2.internal.utils.type_protocols import (
        PlotlyFigureLike,
    )
...
def get_image_content(image: object, autoscale: bool = True) -> bytes | None:
    content = _image_to_bytes(image, autoscale)

    return content


def get_html_content(chart: object, **kwargs: bool) -> str | None:
    content = _to_html(chart, **kwargs)

    return content


def get_pickle_content(obj: object) -> bytes | None:
    content = _export_pickle(obj)

    return content


def _image_to_bytes(image: object, autoscale: bool) -> bytes:
    if image is None:
...
def _to_html(chart: object, **kwargs: bool) -> str:
    if _is_matplotlib_pyplot(chart):
        chart = chart.gcf()  # type: ignore[union-attr]
...
def _matplotlib_to_plotly(chart: object) -> PlotlyFigureLike:
...
def _get_numpy_as_image(array: np.ndarray, autoscale: bool) -> bytes:
    ...
    if len(array.shape) == 3:
        ...
        if array.shape[2] in (3, 4):
            return _get_pil_image_data(pilimage_fromarray(array.astype(numpy_uint8)))
    raise ValueError(
        "Incorrect size of numpy.ndarray. Should be 2-dimensional or3-dimensional with 3rd dimension of size 1, 3 or 4."
    )
...
def _warn_about_incorrect_image_data_range(array_min: float, array_max: float) -> None:
...
def _get_figure_image_data(figure: object) -> bytes:
    if figure.__class__.__name__ == "Axes":
        figure = figure.figure  # type: ignore[union-attr]
    with io.BytesIO() as image_buffer:
        figure.savefig(image_buffer, format="png", bbox_inches="tight")  # type: ignore[union-attr]
        return image_buffer.getvalue()


def _is_torch_tensor(image: object) -> bool:
    return (
...
def _is_tensorflow_tensor(image: object) -> bool:
    return (
...
def _is_matplotlib_pyplot(chart: object) -> bool:
...
def is_matplotlib_figure(image: object) -> bool:
    return image.__class__.__module__.startswith("matplotlib.") and image.__class__.__name__ in ["Figure", "Axes"]


def is_plotly_figure(chart: object) -> bool:
    return chart.__class__.__module__.startswith("plotly.") and chart.__class__.__name__ == "Figure"


def is_altair_chart(chart: object) -> bool:
    return chart.__class__.__module__.startswith("altair.") and "Chart" in chart.__class__.__name__


def is_bokeh_figure(chart: object) -> bool:
    return chart.__class__.__module__.startswith("bokeh.") and chart.__class__.__name__.lower() == "figure"


def is_seaborn_figure(chart: object) -> bool:
    return (
        chart.__class__.__module__.startswith("seaborn.axisgrid") and chart.__class__.__name__ in SEABORN_GRID_CLASSES
    )


def is_pandas_dataframe(table: object) -> bool:
    return isinstance(table, DataFrame)


def _export_pandas_dataframe_to_html(table: DataFrame) -> str:
    buffer = StringIO(table.to_html())
    buffer.seek(0)
    return buffer.getvalue()


def _export_plotly_figure(image: object, **kwargs: bool) -> str:
    buffer = StringIO()
    image.write_html(buffer, include_plotlyjs=kwargs.get("include_plotlyjs", True))  # type: ignore[union-attr]
    buffer.seek(0)
    return buffer.getvalue()


def _export_altair_chart(chart: object) -> str:
    buffer = StringIO()
    chart.save(buffer, format="html")  # type: ignore[union-attr]
    buffer.seek(0)
    return buffer.getvalue()


def _export_bokeh_figure(chart: object) -> str:
...
def _export_pickle(obj: object) -> bytes:
    buffer = BytesIO()
...
def _export_seaborn_figure(chart: object) -> str:
    return _export_plotly_figure(_matplotlib_to_plotly(chart.figure))  # type: ignore[union-attr]
