...
class UploadFile(Operation):
    ...
    @classmethod
    def of_file(cls, value: File, attribute_path: list[str], operation_storage: OperationStorage) -> UploadFile:
        if value.file_type is FileType.LOCAL_FILE:
            operation = UploadFile(
                path=attribute_path,
                ext=value.extension,
                file_path=str(Path(value.path).resolve()),
            )
    ...
    @staticmethod
    def get_tmp_file_name(attribute_path: list[str], extension: str) -> str:
...
class LogSeriesValue(Generic[T]):
    ...
    def to_dict(self, value_serializer: Callable[[T], Any] = lambda x: x) -> dict:
        return {"value": value_serializer(self.value), "step": self.step, "ts": self.ts}

    @staticmethod
    def from_dict(data: dict, value_deserializer: Callable[[Any], T] = lambda x: x) -> LogSeriesValue[T]:
...
class ImageValue:
    ...
    @staticmethod
    def serializer(obj: ImageValue) -> dict[str, str | None]:
        return {"data": obj.data, "name": obj.name, "description": obj.description}

    @staticmethod
    def deserializer(obj: dict | str | None) -> ImageValue:
        if obj is None:
