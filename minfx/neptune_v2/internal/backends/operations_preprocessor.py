...
class OperationsPreprocessor:
    def __init__(self):
        self._accumulators: dict[str, _OperationsAccumulator] = {}
...
class _OperationsAccumulator(OperationVisitor[None]):
    def __init__(self, path: list[str]):
        self._path = path
        self._type: _DataType | None = None
        self._delete_ops = []
        self._modify_ops = []
        self._config_ops = []
        self._errors = []
        ...
        if self._type and self._type != expected_type:
            ...
            self._errors.append(
                MetadataInconsistency(
                    f"Cannot perform {op.__class__.__name__} operation on {path_to_str(self._path)}: Attribute is not a {expected_type.value}"
                )
            )
        else:
    ...
    @staticmethod
    def _assign_modifier() -> Callable[[list[Operation], Operation], list[Operation]]:
        return lambda ops, new_op: [new_op]

    @staticmethod
    def _clear_modifier() -> Callable[[list[Operation], Operation], list[Operation]]:
        return lambda ops, new_op: [new_op]
    ...
    @staticmethod
    def _add_modifier() -> Callable[[list[Operation], Operation], list[Operation]]:
        # We do not optimize it on client side for now. It should not be often operation.
        return lambda ops, op: [*ops, op]

    @staticmethod
    def _remove_modifier() -> Callable[[list[Operation], Operation], list[Operation]]:
        # We do not optimize it on client side for now. It should not be often operation.
        return lambda ops, op: [*ops, op]
