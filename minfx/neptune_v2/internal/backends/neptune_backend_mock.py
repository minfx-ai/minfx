...
class NeptuneBackendMock(NeptuneBackend):
    ...
    def __init__(
        ...
        self._containers: dict[(UniqueId, ContainerType), ContainerStructure[Value, dict]] = {}
        self._next_run = 1  # counter for runs
        self._next_model_version = defaultdict(lambda: 1)  # counter for model versions
        self._artifacts: dict[tuple[str, str], list[ArtifactFileData]] = {}
    ...
    def _get_container(self, container_id: UniqueId, container_type: ContainerType) -> ContainerStructure[Value, dict]:
    ...
    def create_run(
        ...
        *,
        ...
        # Use external IDs if provided (for multi-backend sync), otherwise generate
    ...
    def _generate_attributes(self, base_path: str | None, values: dict):
    ...
    def download_file(
        ...
        target_path = str(
            Path(destination or (path[-1] + ("." + value.extension if value.extension else ""))).resolve()
        )
    ...
    def get_file_attribute(self, container_id: str, container_type: ContainerType, path: list[str]) -> FileAttribute:
        ...
        return FileAttribute(
            name=Path(val.path).name if val.file_type is FileType.LOCAL_FILE else "",
    ...
    def _get_attribute_values(
        self, value_dict: dict[str, Any], path_prefix: list[str]
    ) -> Generator[tuple[str, AttributeType, Any], None, None]:
    ...
    class NewValueOpVisitor(OperationVisitor[Optional[Value]]):
        ...
        def visit_assign_float(self, op: AssignFloat) -> Value | None:
        ...
        def visit_assign_int(self, op: AssignInt) -> Value | None:
        ...
        def visit_assign_bool(self, op: AssignBool) -> Value | None:
        ...
        def visit_assign_string(self, op: AssignString) -> Value | None:
        ...
        def visit_assign_datetime(self, op: AssignDatetime) -> Value | None:
        ...
        def visit_assign_artifact(self, op: AssignArtifact) -> Value | None:
        ...
        def visit_track_files_to_artifact(self, _: TrackFilesToArtifact) -> Value | None:
            if self._current_value is not None and not isinstance(self._current_value, Artifact):
                raise self._create_type_error("save", Artifact.__name__)
            return Artifact(self._artifact_hash)

        def visit_clear_artifact(self, _: ClearArtifact) -> Value | None:
        ...
        def visit_upload_file(self, op: UploadFile) -> Value | None:
        ...
        def visit_upload_file_content(self, op: UploadFileContent) -> Value | None:
            if self._current_value is not None and not isinstance(self._current_value, File):
        ...
        def visit_upload_file_set(self, op: UploadFileSet) -> Value | None:
        ...
        def visit_log_floats(self, op: LogFloats) -> Value | None:
        ...
        def visit_log_strings(self, op: LogStrings) -> Value | None:
            raw_values = [x.value for x in op.values]
            if self._current_value is None:
        ...
        def visit_log_images(self, op: LogImages) -> Value | None:
        ...
        def visit_clear_float_log(self, op: ClearFloatLog) -> Value | None:
            if self._current_value is None:
                return FloatSeries([])
            if not isinstance(self._current_value, FloatSeries):
        ...
        def visit_clear_string_log(self, op: ClearStringLog) -> Value | None:
            if self._current_value is None:
                return StringSeries([])
            if not isinstance(self._current_value, StringSeries):
        ...
        def visit_config_float_series(self, op: ConfigFloatSeries) -> Value | None:
            if self._current_value is None:
        ...
        def visit_add_strings(self, op: AddStrings) -> Value | None:
            if self._current_value is None:
        ...
        def visit_remove_strings(self, op: RemoveStrings) -> Value | None:
            if self._current_value is None:
        ...
        def visit_clear_string_set(self, op: ClearStringSet) -> Value | None:
            if self._current_value is None:
                return StringSet(set())
            if not isinstance(self._current_value, StringSet):
        ...
        def visit_delete_files(self, op: DeleteFiles) -> Value | None:
            if self._current_value is None:
                return FileSet([])
            if not isinstance(self._current_value, FileSet):
        ...
        def visit_delete_attribute(self, op: DeleteAttribute) -> Value | None:
            if self._current_value is None:
                raise MetadataInconsistency(f"Cannot perform delete operation on {self._path}. Attribute is undefined.")
            return None

        def visit_copy_attribute(self, op: CopyAttribute) -> Value | None:
            return op.resolve(self._backend).accept(self)

        def _create_type_error(self, op_name: str, expected: str) -> MetadataInconsistency:
            return MetadataInconsistency(
                f"Cannot perform {op_name} operation on {self._path}. Expected {expected}, {type(self._current_value)} found."
            )
