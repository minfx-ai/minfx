...
class WebsocketSignalsBackgroundJob(BackgroundJob):
    def __init__(self, ws_factory: WebsocketsFactory):
        ...
        self._thread: WebsocketSignalsBackgroundJob._ListenerThread | None = None
        self._started = False
    ...
    class _ListenerThread(Daemon):
        ...
        def _handle_stop(self, msg_body: dict | None) -> None:
        ...
        def _handle_abort(self, msg_body: dict | None) -> None:
            msg_body = msg_body or {}
            if not isinstance(msg_body, dict):
                logger.error("Malformed websocket signal: body is %s, should be dict", type(msg_body))
                return
        ...
        @staticmethod
        def _is_heartbeat(message: str) -> bool:
