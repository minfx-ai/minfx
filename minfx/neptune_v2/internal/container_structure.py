...
def _default_node_factory(path: list[str]) -> dict:
    return {}


class ContainerStructure(Generic[T, Node]):
    def __init__(self, node_factory: Callable[[list[str]], Node] | None = None):
    ...
    def _iterate_node(self, node: Node, path_prefix: list[str]) -> Generator[tuple[list[str], T], None, None]:
    ...
    def get(self, path: list[str]) -> T | Node | None:
        ref = self._structure

        for index, part in enumerate(path):
            if not isinstance(ref, self._node_type):
                raise MetadataInconsistency(
                    f"Cannot access path '{path_to_str(path)}': '{path_to_str(path[:index])}' is already defined as an attribute, "
                    "not a namespace"
                )
            if part not in ref:
                return None
    ...
    def set(self, path: list[str], attr: T) -> None:
        ...
        for idx, part in enumerate(location):
            ...
            if not isinstance(ref, self._node_type):
                raise MetadataInconsistency(
                    f"Cannot access path '{path_to_str(path)}': '{part}' is already defined as an attribute, "
                    "not a namespace"
                )
    ...
    def _pop_impl(self, ref: Node, sub_path: list[str], attr_path: list[str]) -> None:
        if not sub_path:
            return
        ...
        if not tail:
            if isinstance(ref[head], self._node_type):
                raise MetadataInconsistency(
                    f"Cannot delete {path_to_str(attr_path)}. It's a namespace, not an attribute."
                )
            del ref[head]
        else:
