...
from minfx.neptune_v2.management.internal.dto import (
...
from minfx.neptune_v2.management.internal.utils import (
    WorkspaceMemberRole,
...
def _get_http_client_and_api_url(api_token: str | None = None) -> tuple[object, str]:
    credentials = Credentials.from_token(api_token=_get_token(api_token=api_token))
    http_client, _client_config, api_url = create_http_client_with_auth(
        credentials=credentials, ssl_verify=ssl_verify(), proxies={}
    )
    return http_client, api_url


def _get_backend_client(api_token: str | None = None) -> SwaggerClientWrapper:
...
def _get_leaderboard_client(api_token: str | None = None) -> SwaggerClientWrapper:
    http_client, api_url = _get_http_client_and_api_url(api_token)
    return create_leaderboard_client(api_url=api_url, http_client=http_client)


def get_project_list(*, api_token: str | None = None) -> list[str]:
...
def _get_projects(backend_client: SwaggerClientWrapper, params: dict[str, Any]) -> list:
...
def _get_workspace_id(backend_client: SwaggerClientWrapper, workspace: str) -> str:
...
@with_api_exceptions_handler
def _get_workspaces(backend_client: SwaggerClientWrapper) -> list:
    return backend_client.api.listOrganizations(**DEFAULT_REQUEST_KWARGS).response().result


@with_api_exceptions_handler
def _create_project(backend_client: SwaggerClientWrapper, project_qualified_name: str, params: dict) -> object:
    try:
...
@with_api_exceptions_handler
def delete_project(project: str, *, workspace: str | None = None, api_token: str | None = None):
...
def get_project_member_list(
    project: str, *, workspace: str | None = None, api_token: str | None = None
) -> dict[str, str]:
...
@with_api_exceptions_handler
def get_workspace_member_list(workspace: str, *, api_token: str | None = None) -> dict[str, str]:
...
def _get_raw_workspace_service_account_list(
    workspace_name: str, api_token: str | None = None
...
@with_api_exceptions_handler
def get_workspace_service_account_list(workspace: str, *, api_token: str | None = None) -> dict[str, str]:
...
def invite_to_workspace(
    *,
    ...
    workspace: str,
    api_token: str | None = None,
    role: WorkspaceMemberRole | str = WorkspaceMemberRole.MEMBER,
...
def trash_objects(
    ...
    *,
    ...
    Examples:
        ...
        >>> management.trash_objects(project="ml-team/classification", ids=["CLS-2", "CLS-3", "CLS-PRETRAINED"])
...
def get_workspace_status(workspace: str, *, api_token: str | None = None) -> dict[str, int]:
